define(function(require) {
	...
	var ManufacturerProductsCollection = Backbone.Collection.extend({
		...
		initialize: function(options) {
			...
			this.id = options.id;
			this.manImg = this.getImageUrl(this.id);

		},

		getImageUrl: function(id) {
			
			var imageUrl = Config.apiUrl + "images/manufacturers/" + id + "/?ws_key=" + Config.secureKey; 
			return imageUrl;
		...
		url: function () {
			...
			url += 		"&filter[price]=[0.01,1000]&filter[id_manufacturer]=" + this.id;
		...
		parse: function(data) {
			...
			try {
				...
				this.valid = false; 
				console.log("Nessun prodotto per questo produttore");
